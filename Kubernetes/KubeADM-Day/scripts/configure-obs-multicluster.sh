...
#
# DEV_TARGET = dev master IP + kube-state-metrics NodePort (30301).
...
nc -vz -w 3 "${DEV_MASTER_IP}" "${KSM_NODEPORT}" </dev/null 2>&1 | tail -1 || echo "WARN: ${DEV_TARGET} blocked — add dev SG TCP ${KSM_NODEPORT} from 10.210.0.0/16"
